...
def construct_graph(training_dir, edges, nodes, target_node_type):
    ...
    for i, edge in enumerate(edges):
        ...
        if src == 'target' and dst == 'target':
            ...
            # Convert edge list to edge_index format (2 x num_edges tensor)
            if edgelist:
                edge_tensor = th.tensor(edgelist, dtype=th.long).t().contiguous()
                edge_index_dict[(src, src + '<>' + dst, dst)] = edge_tensor
                
                # Add reverse edges
                if rev_edgelist:
                    rev_edge_tensor = th.tensor(rev_edgelist, dtype=th.long).t().contiguous()
                    edge_index_dict[(dst, dst + '<>' + src, src)] = rev_edge_tensor
    ...
    # add self relation for target nodes
    target_node_ids = list(id_to_node[target_node_type].values())
    if target_node_ids:
        self_edges = [[i, i] for i in target_node_ids]
        self_edge_tensor = th.tensor(self_edges, dtype=th.long).t().contiguous()
        edge_index_dict[('target', 'self_relation', 'target')] = self_edge_tensor
    ...
    # Store node counts for each type
    node_counts = {}
    for ntype, node_dict in id_to_node.items():
        if ntype != target_node_type:
            node_counts[ntype] = len(node_dict)
    
    # Add target node features
    data['target'].x = th.from_numpy(features)
    node_counts['target'] = len(id_to_node[target_node_type])
    
    print("Constructed heterograph with the following node types: {}".format(list(node_counts.keys())))
    print("Edge types: {}".format(list(edge_index_dict.keys())))
    print("Number of nodes of type target : {}".format(node_counts['target']))
