...
def parse_args():
    ...
    # Distributed feature engineering arguments
    parser.add_argument('--use-distributed-features', type=lambda x: (str(x).lower() in ['true', '1', 'yes']),
                        default=False, help='Use distributed feature engineering')
    parser.add_argument('--feature-dir', type=str, default='./features',
                       help='Directory containing pre-extracted features')
    parser.add_argument('--n-feature-workers', type=int, default=None,
                       help='Number of workers for feature extraction (default: CPU count)')
    parser.add_argument('--enhance-features', type=lambda x: (str(x).lower() in ['true', '1', 'yes']),
                        default=False, help='Enhance existing features with advanced extractors')
